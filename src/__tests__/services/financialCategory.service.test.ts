...
describe("financialCategoryService.archive", () => {
  ...
  it("archives a category with no active children", async () => {
    ...
    await financialCategoryService.archive("parent-1", "ws-1")

    expect(financialCategoryRepository.update).toHaveBeenCalledWith("parent-1", "ws-1", { isArchived: true })
  })
  ...
  it("blocks archiving (FINANCIAL_CATEGORY_HAS_CHILDREN) while active subcategories exist", async () => {
    ...
    await expect(
      financialCategoryService.archive("parent-1", "ws-1"),
    ).rejects.toMatchObject({ code: ErrorCode.FINANCIAL_CATEGORY_HAS_CHILDREN })
    expect(financialCategoryRepository.update).not.toHaveBeenCalled()
  })
})
